 ...
 * clang E1code3 fft.o -o <executable name> -lgsl -lgslcblas
 */
...
#define N_POINTS 250 //The nyquist frequency is 5 per time-interval
#define dt 0.1
...
void read_data(char *fname, double *time_array, double *signal) {
    ...
    /* if file not found, error out and exit code 1 */
    if (fp == NULL) {
        perror("error: ");
        exit(1);
    }
 ...
 */
void write_to_file(char *fname, double *frequencies, double *spectrum, int n_points) {
...
int main(int argc, char **argv) {
    ...
    powerspectrum_shift(fftd_data, N_POINTS); //Adding these two will shift the spectrum!
