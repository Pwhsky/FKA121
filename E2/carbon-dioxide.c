...
void calc_acc(double *a, double *u, double *m, double kappa, int size_of_u)
{
    ...
    for (i = 1; i < size_of_u - 1; i++) {
        a[i] = - kappa*(u[1] - u[0])/m[0] + kappa*(u[size_of_u - 1]-u[size_of_u - 2])/m[size_of_u - 1];
    }
}
...
void velocity_verlet(int n_timesteps, int n_particles, double *v, double *q_1, double *q_2, double *q_3,
    ...
    for (int i = 1; i < n_timesteps + 1; i++) {
        ...
        q_2[i] = q[1];
        q_3[i] = q[2];
        kinetic[i]    =   0.5*(m[0]*pow(v[0],2) + m[1]*pow(v[1],2) + m[2]*pow(v[2],2));
...
int main(){
    ...
    int n_particles  = 3;
    double kappa = 1600/16.0218; double dt = 0.00005; double carbon_mass = 12.01/m_asu; double oxygen_mass = 15.999/m_asu;
